...
class Adminster extends Common {
	...
	public function index(){
			...
			if(!empty($params['login'])){
						$where['adminster_login']=["like","%".$params['login']."%"];
			}else{
						$params['login']="";
			}
			if(!empty($params['email'])){
						$where['adminster_email']=["like","%".$params['email']."%"];
			}else{
						$params['email']="";
			}
			if(!empty($params['state'])){
					$where['adminster_state']=$params['state'];
			}else{
					$params['state']="";
			}
			if(!empty($params['group'])){
					$groups['group_id']=$params['group'];
			}else{
					$params['group']="";
			}

			$params['page']=Request::instance()->param('page')?:1;
			$adminster_list= Adminsters::haswhere('profile',$groups)->where($where)->paginate(Config::get('page_size'), false, [
					'query'=>$params,
				]);
	...
	public function add(){
		if(Request::instance()->isPost()){
			  ...
			  $adminster->adminster_pwd=encryption(Request::instance()->post('login_passwd'),$code);
				...
				Session::set('jump_msg',['type'=>'success','msg'=>'信息已经添加','data'=>'']);
	...
	public function edit(){
		if(Request::instance()->isPost()){
				...
				Session::set('jump_msg',['type'=>'success','msg'=>'信息已经更新','data'=>'']);
				$this->redirect($this->history['1']);
		}
		return $this->get_form();
	}
