 ...
 */
 class Huilianluodi{
    ...
    public function card_bind_new($agentId,$merId,$MemberCreditcard,$passageway){
        $data=array(
            'version'=>$this->version,
            'serviceUri'=>'YQ0005',//交易代码      str (8) 是   YQ0001
            'charset'=>'UTF-8',//编码格式     str (8) 是   UTF-8
            'signType'=>'RSA',//签名方式        str (8)     是   RSA
            'nonceStr'=>make_rand_code(),//随机字符串   nonceStr    str (32)    是   随机字符串
            'agentId'=>$agentId,//代理商号     str (8) 是   受理方预分配的渠道代理商标识
    ...
    public function pay($value,$passageway_mech){
        ...
        if(!$value['order_platform_no'] || $value['order_status']!=1){
            $update_order['order_platform_no']=$value['order_platform_no']=get_plantform_pinyin().$member_base->member_mobile.make_rand_code();
        ...
        $data=array(
            ...
            // 'goods'=>System::getName('sitename').'-'.$member_base->member_mobile,//商品描述   str (32)    是   
